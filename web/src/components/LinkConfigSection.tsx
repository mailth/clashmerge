...
export default function LinkConfigSection({
  isActive,
  ...
  const [editingConfig, setEditingConfig] = useState<LinkConfig | null>(null);
  ...
  const loadData = async () => {
    try {
      ...
      const [links, merges] = await Promise.all([
        apiService.getLinkConfigs(),
        apiService.getMergeConfigs(),
      ]);

      setLinkConfigs(links);
      setMergeConfigs(merges);
    } catch (err) {
  ...
  const handleEdit = (config: LinkConfig) => {
    setEditingConfig(config);
    setShowModal(true);
  };

  const handleDelete = async (id: number) => {
    if (!confirm("确定要删除这个链接配置吗？")) return;

    try {
      await apiService.deleteLinkConfig(id);
      await loadData();
      alert("删除成功");
    } catch (err) {
  ...
  if (!isActive) {
    return <div id='link-config' className='section hidden'></div>;
  }

  return (
    <div id='link-config' className='section'>
      <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6'>
        <div>
          <h2 className='text-xl font-semibold text-gray-800'>链接配置管理</h2>
          <p className='text-gray-600 text-sm mt-1'>管理您的 Clash 链接配置</p>
        </div>
        <button
      ...
      {!loading && !error && linkConfigs.length === 0 && (
